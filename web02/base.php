...
class DB{
    ...
    private $dsn = "mysql:host=localhost;charset=utf8;dbname=db01_web02"; //不能有空格
    private $root ='root';
    private $password ="";
    private $table;
    private $pdo;
    ...
    // 檢查有無今日日期 //判斷是否新增當日的資料
    // 如果無今日日期>增加今日日期,total為0
    if(!$Total->count(['date'=>date("Y-m-d")])){
        $Total->save(['date'=>date("Y-m-d"),'total'=>0]);
    }

    // 檢查有無$_SESSION['total']資料 //判斷是否要增加session
    if(!isset($_SESSION['total'])){
        // 到total資料表撈取整筆今日資料=>$todayTotal
        // total人數加1 => $todayTotal=$todayTotal['total']+1
        // 回存total資料表
        $todayTotal=$Total->find(['date'=>date("Y-m-d")]);
        $todayTotal['total']++;
        $Total->save($todayTotal);

        // 設定$_SESSION['total']有無狀態
        $_SESSION['total']=1;
